...
public class Bishop {
	...
	public Bishop(char x, int y, int z, int i) {
		this.symbol = x;
		this.currentR = y;
		this.currentC = z;
		this.team = i;
	...
	public int possibleMoves(char[][] x, int Kr, int Kc) {
		...
		for (int i = this.currentR - 1; i >= 0; i--) {
			...
			if (this.team == 1 && j < 8 && j >= 0) {
				if (x[i][j] != (char) 9818 && x[i][j] != (char) 9819 && x[i][j] != (char) 9820 && x[i][j] != (char) 9821
				...
				if (x[i][j] != (char) 9812 && x[i][j] != (char) 9813 && x[i][j] != (char) 9814 && x[i][j] != (char) 9815
						&& x[i][j] != (char) 9816 && x[i][j] != (char) 9817) {
					test[i][j] = (char) 9815;
					checked = isChecked(test, Kr, Kc);
					if (checked == 0) {
